...
class ORIGINAL_AMT(FactorBase):
    def __init__(self, frequency):
        super(ORIGINAL_AMT, self).__init__(frequency)
    ...
    def compute(self, start_date=None, end_date=None):
        ...
        for contract in self.contracts:
            ...
            temp = self.api.market_data(table_name, 'datetime,turnover', start_date, end_date)
            if len(temp) > 0:
                temp = temp.rename(columns={'turnover': contract})
