...
class ORIGINAL_VOLUME(FactorBase):
    def __init__(self, frequency):
        super(ORIGINAL_VOLUME, self).__init__(frequency)
    ...
    def compute(self, start_date=None, end_date=None):
        ...
        for contract in self.contracts:
            ...
            temp = self.api.market_data(table_name, 'datetime,volume', start_date, end_date)
            if len(temp) > 0:
                temp = temp.rename(columns={'volume': contract})
