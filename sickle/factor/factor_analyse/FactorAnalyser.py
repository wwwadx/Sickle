...
class FactorAnalyser:
    ...
    def factor_to_portfolio_ls_weight(self, fac_df, side, period):
        ...
        count_1 = temp_count[temp_count > 1]
        if len(count_1) > 0:
            fac = fac[fac.index >= count_1.index[0]]
    ...
    def factor_to_portfolio_ls_count(self, fac_df, side, period, count):
        """
        因子构建多空组合, 做指定数量,等权
        Args:
            fac_df: 矩阵形式存储的因子DataFrame
        ...
        fac.replace(-np.inf, np.nan, inplace=True)
        fac = fac.dropna(how='all', axis=0)
        fac = fac[fac.max(1) != fac.min(1)]
        select_df = pd.DataFrame()
        # 对因子进行过滤，从开始剔除截面上少于10个品种的数据（没意义）
        temp_count = fac.count(1)
        count_1 = temp_count[temp_count >= 10]
        if len(count_1) > 0:
            fac = fac[fac.index >= count_1.index[0]]
            # 对因子覆盖度进行过滤， 平均覆盖度小于截面信息一半的因子剔除（没意义）
            if len(fac) > 1 and fac.count(1).mean() > fac.shape[1] // 2:
    ...
    #TODO
    def factor_to_portfolio_ls_count_lowturn(self, fac_df, side, period, count, percent):
        """
        因子构建多空组合, 做指定数量,等权,给定换手缓冲，降低换手
        Args:
            fac_df: 矩阵形式存储的因子DataFrame
            side: 因子方向, int, 1或-1
            period: 调仓周期，是因子频率的period倍
